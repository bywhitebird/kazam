import { type Diagnostic, type LanguageServerPlugin, type Service, type TextDocument, createConnection, startLanguageServer } from '@volar/language-server/node'
import createTypescriptService from 'volar-service-typescript'

import { KazFile, language } from './language'
...
const plugin: LanguageServerPlugin = (): ReturnType<LanguageServerPlugin> => ({
  ...
  resolveConfig(config) {
    ...
    config.services.kaz ??= (context): ReturnType<Service> => ({
